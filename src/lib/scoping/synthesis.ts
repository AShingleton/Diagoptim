...
export async function runSynthesis(
  ...
  await prisma.a3Report.create({
    data: {
      ...
      followUp: { criteres: synthesis.cahierDesCharges.criteresDeRecette, strategic: synthesis.strategic ?? null, automatisation: synthesis.automatisation ?? null, kpis: synthesis.kpis ?? null, operatingModel: synthesis.operatingModel ?? null } as unknown as object,
    },
  });
...
export async function getStoredSynthesis(prisma: PrismaClient, projectId: string): Promise<ScopingSynthesis | null> {
  ...
  const followUp = (a3.followUp ?? {}) as { strategic?: ScopingSynthesis["strategic"]; automatisation?: ScopingSynthesis["automatisation"]; kpis?: ScopingSynthesis["kpis"]; operatingModel?: ScopingSynthesis["operatingModel"] };
  ...
  const operatingModel = followUp.operatingModel ?? undefined;
  return {
